...
int mi_disp_lhbm_fod_thread_create(struct disp_feature *df, int disp_id)
{
	...
	lhbm_fod->dsi = dsi;
	lhbm_fod->dsi->mi_cfg.lhbm_ui_ready_delay_frame = 3;
...
static int mi_disp_lhbm_fod_set_disp_param(struct disp_lhbm_fod *lhbm_fod,
					   u32 lhbm_value)
{
	struct mtk_dsi *dsi = NULL;
...
void mi_disp_lhbm_animal_status_update(struct mtk_dsi *dsi, bool is_layer_exit)
{
	struct disp_feature_ctl ctl;
...
int mi_disp_set_fod_queue_work(u32 fod_status, bool from_touch)
{
	 ...
	 */
	DISP_INFO("FodEngine change to mi_disp_ioctl_set_local_hbm\n");
	return 0;
