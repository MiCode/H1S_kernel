...
#define pr_fmt(fmt)	"mi-dsi-panel:" fmt
...
unsigned int screen_state = 2;
EXPORT_SYMBOL(screen_state);
...
/*
	if(doze_brightness == 0 && screen_state == 2){
		...
		DDPMSG("%s, blank:%u",__func__, data);
        }
*/
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_LOCAL_HBM:
		DISP_INFO("DISP_FEATURE_LOCAL_HBM:%d\n", ctl->feature_val);
