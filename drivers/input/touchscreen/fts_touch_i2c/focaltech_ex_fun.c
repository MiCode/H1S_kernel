...
#if 0
#define TP_PROXIMITY "tp_proximity"

typedef struct lct_tp{
  	int enable_tp_proximity_flag;
...
#if 0
#if LCT_TP_VIRTUAL_PSENSOR
ssize_t set_lct_tp_proximity_status(int value)
{
  	lct_tp_p->enable_tp_proximity_flag = value;
	FTS_INFO("into lct_set_proximity_status,value = %d\n",value);
  	return 0;
}
...
static ssize_t lct_proc_tp_proximity_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
  	...
  	FTS_DEBUG("lct_tp_p->enable_tp_proximity_flag = %d\n",lct_tp_p->enable_tp_proximity_flag);
  	cnt = sprintf(page, "%d\n", lct_tp_p->enable_tp_proximity_flag);
  	cnt = simple_read_from_buffer(buf, size, ppos, page, cnt);
  	if (*ppos != cnt)
  		*ppos = cnt;
  	FTS_INFO("Touchpad status : %d", page);
	...
	int value = 0;
 	...
 	ssize_t cnt = 0;
  	...
  	if (sscanf(page, "%u", &input) != 1)
		return -EINVAL;
	ret = set_lct_tp_proximity_status(value);
  	return ret;
}

static const struct proc_ops lct_proc_tp_proximity_fops = {
	.proc_write = lct_proc_tp_proximity_write,
	.proc_read = lct_proc_tp_proximity_read,
};

static int fts_tp_proximity_init(void)
{
	int ret = 0;
	FTS_TEST_FUNC_ENTER();
  	lct_tp_p = kzalloc(sizeof(lct_tp_t), GFP_KERNEL);
  	if (IS_ERR_OR_NULL(lct_tp_p)){
  		FTS_INFO("kzalloc() request memory failed!\n");
 		return -ENOMEM;
        }
  	lct_tp_p->enable_tp_proximity_flag = true;
	lct_tp_p->proc_entry_tp = proc_create(TP_PROXIMITY, 0666, NULL, &lct_proc_tp_proximity_fops);
	if (lct_tp_p->proc_entry_tp == NULL) {
		FTS_INFO("create proc/%s Failed!\n", TP_PROXIMITY);
		return -ENOMEM;
	} else {
...
static void fts_tp_proximity_proc_exit(void)
{
	if (lct_tp_p->proc_entry_tp != NULL) {
		remove_proc_entry(TP_PROXIMITY, NULL);
...
int lct_fts_get_lockdown_info(void)
{
    ...
    mutex_unlock(&input_dev->mutex);
    update_lct_tp_lockdown_info(tp_lockdown_info_buf);

#if FTS_ESDCHECK_EN
    fts_esdcheck_proc_busy(ts_data, 0);
#endif
    FTS_INFO("update tp_lockdown_info: %s", tp_lockdown_info_buf);
...
int lct_fts_get_tpfwver(const char *cmd)
{
    ...
    {
        num_read_chars = snprintf(buf, 64, "[Vendor]samsung,[FW]0x%02x,[IC]ft3519t\n", fwver);
    }
    mutex_unlock(&input_dev->mutex);
    update_lct_tp_info(buf);
    FTS_INFO("update tp_info:%s", buf);

    return 0;
}
#endif // TP_INFO_TP_LOCKDOWN end
...
{
    int ret = 0;
    ret = init_lct_tp_info("[Vendor]samsung,[FW]unknow,[IC]ft3519t\n");
    if (ret)
    {
        FTS_ERROR("create /proc/tp_info fail");
        goto err_init_lct_tp_info_fail;
    }
    else
    {
        set_lct_tp_info_callback(lct_fts_get_tpfwver);
        FTS_INFO("create /proc/tp_info succeeded");
    }

    /***  touch not create /proc/tp_lockdown_info start ****/
    ret = init_lct_tp_lockdown_info();
    if (ret)
    {
        FTS_ERROR("create /proc/tp_lockdown_info fail");
        goto err_init_lct_tp_lockdown_info_fail;
    }
    else
    {
        set_lct_tp_lockdown_info_callback(lct_fts_get_lockdown_info);
        FTS_INFO("create /proc/tp_lockdown_info succeeded");
    }
    /***  touch not create /proc/tp_lockdown_info end ****/
...
#if 0
#if LCT_TP_VIRTUAL_PSENSOR
	ret = fts_tp_proximity_init();
	if (ret < 0) {
		FTS_ERROR("fts_tp_proximity_init Failed!");
		//goto err_init_lct_tp_grip_area_fail;

	} else {
		FTS_INFO("fts_tp_proximity_init Succeeded!");
        }
#endif
#endif
...
err_init_lct_tp_lockdown_info_fail:
    uninit_lct_tp_lockdown_info();
#endif
...
#if LCT_TP_VIRTUAL_PSENSOR
    ts_data->fw_loading = true;
    uninit_lct_tp_proximity_switch();
    // fts_tp_proximity_proc_exit();
#endif
    return 0;
}
