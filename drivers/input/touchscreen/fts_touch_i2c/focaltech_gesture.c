...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    if ((gesture == KEY_GESTURE_AOD) && fts_data->aod_changed){
        //aod on to report KEY_GOTO
        FTS_DEBUG("AOD Code=%d", gesture);
        input_report_key(input_dev, gesture, 1);
        input_sync(input_dev);
        input_report_key(input_dev, gesture, 0);
        input_sync(input_dev);
   }
   else if ((gesture == KEY_GESTURE_AOD) && fts_data->fod_icon_status && !fts_data->aod_changed && fts_data->fod_support){
        //aod off & fod_icon open to report KEY_GOTO
        FTS_DEBUG("Click Code=%d", gesture);
        input_report_key(input_dev, gesture, 1);
        input_sync(input_dev);
        input_report_key(input_dev, gesture, 0);
        input_sync(input_dev);
   }
   else if ((gesture == KEY_GESTURE_U) && fts_data->gesture_status == 1) {
        //Double click open report KEY_WAKEUP
...
int fts_gesture_readdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    if (!ts_data->gesture_support ||(!ts_data->gesture_support && !ts_data->aod_changed)) {
...
#if FTS_FOD_EN
    if (gesture->gesture_id == FTS_FOD_PRESSED)
...
int fts_fod_reg_write(u8 mask, bool enable)
{
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        // FTS_INFO("%s reg_value:%d reg_value_last_time:%d\n",__func__,reg_value,reg_value_last_time);
        reg_value_last_time = reg_value;
        // FTS_INFO("%s mask:%d,enable:%d,reg_value_last_time:%d\n",__func__,mask,enable,reg_value,reg_value_last_time);
        if (enable)
            reg_value |= mask;
        else
            reg_value &= ~mask;
        /* If the value in the register is equal to the modified value, skip writing to the register */
        // FTS_INFO("%s if reg_value == reg_value_last_time reg_value:%d,reg_value_last_time:%d\n",__func__,reg_value,reg_value_last_time);
    ...
    FTS_INFO("[GESTURE]Write fod reg:%d success!\n", reg_value);
    return 0;
}
