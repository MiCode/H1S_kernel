...
#define FTS_PROC_TP_DIFFER "fts_tp_csv_test"
...
//add fts_test_txt start
static struct proc_dir_entry *tp_result_proc;
#define FTS_PROC_TP_DIFFER_TXT "fts_tp_txt_result"

static ssize_t tp_result_proc_write(
    struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    FTS_TEST_FUNC_EXIT();
    return count;
...
static int fts_result_proc_show(struct seq_file *m, void *v)
{
    seq_printf(m, "%s",fts_ftest->txt_buffer);
    return 0;
}

static int fts_result_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, fts_result_proc_show, NULL);
}


static const struct proc_ops tp_result_proc_fops =
{
    .proc_open     = fts_result_proc_open,
    .proc_read     = seq_read,
    .proc_write  = tp_result_proc_write,
};
//add fts_test_txt end
...
static int fts_test_func_init(struct fts_ts_data *ts_data)
{
    ...
    tp_result_proc = proc_create(FTS_PROC_TP_DIFFER_TXT, 0444, NULL, &tp_result_proc_fops);
    if (tp_result_proc == NULL) {
        FTS_TEST_ERROR("[focal] %s() - ERROR: create fts_tp_differ_txt proc()  failed.",  __func__);
    }
...
#define CONF_MULTIPLETEST_INI "Conf_MultipleTest_ft3519t.ini"
int lct_tp_selftest_all(void)
{
    ...
    ts_data->selftest_status = 1;
    if (ts_data->suspended) {
        FTS_INFO("In suspend, no test, return now");
        ts_data->selftest_status = 0;
        return -EINVAL;
    }
...
int fts_test_init(struct fts_ts_data *ts_data)
{
    ...
    memset(fts_ftest->csv_buffer, 0, CSV_BUFFER_LEN);
    memset(fts_ftest->txt_buffer, 0, TXT_BUFFER_LEN);
...
int fts_test_exit(struct fts_ts_data *ts_data)
{
    ...
    if (tp_test_proc != NULL) {
        ...
        tp_test_proc = NULL;
        FTS_INFO("remove /proc/%s\n",FTS_PROC_TP_DIFFER);
    }

    if (tp_result_proc != NULL) {
        remove_proc_entry(FTS_PROC_TP_DIFFER_TXT,NULL);
        tp_result_proc = NULL;
        FTS_INFO("remove /proc/%s\n",FTS_PROC_TP_DIFFER_TXT);
    }
