...
static struct xiaomi_touch xiaomi_touch_dev = {
	...
	.fod_press_status_mutex = __MUTEX_INITIALIZER(xiaomi_touch_dev.fod_press_status_mutex),  //FOD
};
...
EXPORT_SYMBOL(get_xiaomi_touch_class);  //FOD
...
int update_fod_press_status(int value)
{
        ...
        if (value != touch_pdata->fod_press_status_value) {
                ...
                //mi_disp_lhbm_fod_set_finger_event(0, value, true);
                sysfs_notify(&xiaomi_touch_dev.dev->kobj, NULL,
                     "fod_press_status");
        }
        mutex_unlock(&xiaomi_touch_dev.fod_press_status_mutex);
...
static DEVICE_ATTR(fod_press_status, (S_IRUGO | S_IWUSR | S_IWGRP),
		fod_press_status_show, NULL);
#endif
...
static struct attribute *touch_attr_group[] = {
	...
	&dev_attr_fod_press_status.attr,  //FOD
	NULL,
};
