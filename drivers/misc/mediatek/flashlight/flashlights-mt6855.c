...
#define CUST_GPIO_DTNAME "mediatek,flashlights_mt6855"
#endif
/* TODO: define driver name */
#define CUST_NAME "flashlights_mt6855"
/* define registers */
...
static int cust_pinctrl_set(int pin, int state)
{
	...
	pr_err("pin(%d) state(%d) level(%d)\n", pin, state,cust_gpio_level_ch1);
	return ret;
}
/******************************************************************************
 * cust operations
...
static int cust_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	printk("robe_debug: %s run. end. success.\n", __func__);
	return 0;
}
...
void flashlights_mt6855_init_robe(void)
{
    printk("robe_debug: %s run. begin.\n", __func__);
...
EXPORT_SYMBOL(flashlights_mt6855_init_robe);
void flashlights_mt6855_exit_robe(void)
{
    printk("robe_debug: %s run. begin.\n", __func__);
...
EXPORT_SYMBOL(flashlights_mt6855_exit_robe);
