...
#define IMX882_EEPROM_WRITE_ID 0xA1
#define IMX882_I2C_SPEED       100
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(void)
{
	...
	if (pinfo == NULL) {
		read_imx882_eeprom(0x1, read_data, 4);
		if (((read_data[3] << 24) |
		     (read_data[2] << 16) |
		     (read_data[1] << 8) |
...
{
	static BYTE IMX882_LRC_data[LRC_SIZE] = { 0 };
	static unsigned int readed_size;
	struct EEPROM_PDAF_INFO *pinfo = get_eeprom_pdaf_info();

	LOG_INF("read imx882 LRC, addr = %d, size = %u\n",
		pinfo->LRC_addr, pinfo->LRC_size);

	mutex_lock(&gimx882_eeprom_mutex);
	if ((readed_size == 0) &&
	    read_imx882_eeprom(pinfo->LRC_addr,
			       IMX882_LRC_data, pinfo->LRC_size)) {
		readed_size = pinfo->LRC_size;
	}
	mutex_unlock(&gimx882_eeprom_mutex);

	memcpy(data, IMX882_LRC_data, pinfo->LRC_size);
	return readed_size;
}
...
{
	static BYTE IMX882_DCC_data[DCC_SIZE] = { 0 };
	static unsigned int readed_size;
	struct EEPROM_PDAF_INFO *pinfo = get_eeprom_pdaf_info();

	LOG_INF("read imx882 DCC, addr = %d, size = %u\n",
		pinfo->DCC_addr, pinfo->DCC_size);

	mutex_lock(&gimx882_eeprom_mutex);
	if ((readed_size == 0) &&
	    read_imx882_eeprom(pinfo->DCC_addr,
			       IMX882_DCC_data, pinfo->DCC_size)) {
		readed_size = pinfo->DCC_size;
	}
	mutex_unlock(&gimx882_eeprom_mutex);

	memcpy(data, IMX882_DCC_data, pinfo->DCC_size);
	return readed_size;
}
