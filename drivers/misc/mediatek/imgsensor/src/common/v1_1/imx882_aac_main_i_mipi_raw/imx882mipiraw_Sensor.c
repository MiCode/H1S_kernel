...
#include "imx882mipiraw_Setting.h"
...
#define PFX "IMX882_aac_main_i_camera_sensor"
/****************************   Modify end	**************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX882_AAC_MAIN_I_SENSOR_ID,
...
static void imx882_get_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void imx882_set_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	imx882_table_write_cmos_sensor(regDa, regNum*2);
}
...
static kal_uint16 imx882_QSC_setting[3072 * 2];
static kal_uint16 imx882_LRC_setting[384 * 2];
...
static void read_sensor_Cali(void)
{
	...
	if ((otp_data[0] == 0x00) && (otp_data[1] == 0x0C) && (otp_data[2] == 0x01) && (otp_data[3] == 0x02)){
		...
		for (idx = 0; idx < 3072; idx++) {
			...
			imx882_QSC_setting[2 * idx + 1] = read_cmos_eeprom_8(addr_qsc);
		}
	} else {
	...
	if((otp_data[4] == 0x00) && (otp_data[5] == 0x0D) && (otp_data[6] == 0x01) && (otp_data[7] == 0x02)){
		...
		for (idx = 0; idx < 192; idx++) {
			imx882_LRC_setting[2 * idx] = sensor_lrc0 + idx;
			imx882_LRC_setting[2 * idx + 1] = read_cmos_eeprom_8(addr_lrc + idx);
			imx882_LRC_setting[2 * idx + 192 * 2] = sensor_lrc1 + idx;
			imx882_LRC_setting[2 * idx + 1 + 192 * 2] = read_cmos_eeprom_8(addr_lrc + 192 + idx);
		}
	} else {
...
static void write_sensor_QSC(void)
{
	...
	if ((otp_flag == OTP_QSC_CUSTOM) || (otp_flag == OTP_QSC_INTERNAL)) {
		imx882_table_write_cmos_sensor(imx882_QSC_setting,sizeof(imx882_QSC_setting) / sizeof(kal_uint16));
		imx882_table_write_cmos_sensor(imx882_LRC_setting,sizeof(imx882_LRC_setting) / sizeof(kal_uint16));
	}
...
#define IMX882_GAIN_TABL_SIZE 432
kal_uint16 imx882_gain_ratio[IMX882_GAIN_TABL_SIZE] = {
	64,
	65,
	66,
	67,
...
kal_uint16 imx882_gain_reg[IMX882_GAIN_TABL_SIZE] = {
	0,
	240,
	496,
	720,
...
#ifdef USE_GAIN_TABLE
	reg_gain = imx882_gain_reg[IMX882_GAIN_TABL_SIZE - 1];
	for (i = 0; i < IMX882_GAIN_TABL_SIZE; i++) {
		if (gain_value <= imx882_gain_ratio[i]) {
			reg_gain = imx882_gain_reg[i];
			break;
		}
	}
#else
...
static void imx882_awb_gain(kal_uint32 r_gain, kal_uint32 b_gain)
{
	UINT32 r_gain_int = 0;
	UINT32 b_gain_int = 0;
...
static void capture_setting(kal_uint16 currefps)
{
	...
	imx882_table_write_cmos_sensor(imx882_capture_30_setting,
		sizeof(imx882_capture_30_setting)/sizeof(kal_uint16));
...
static kal_uint32 seamless_switch(enum MSDK_SCENARIO_ID_ENUM scenario_id, uint32_t *ae_ctrl)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	{
		...
		FMC_GPH_START;
		imx882_table_write_cmos_sensor(addr_data_pair_fmc_pre,
	...
	case MSDK_SCENARIO_ID_CUSTOM3:
	{
		...
		FMC_GPH_START;
		imx882_table_write_cmos_sensor(addr_data_pair_fmc_cus3,
...
#define IMX882_AAC_EEPROM_I2C_ADDR 0xA2
static kal_uint16 read_imx882_aac_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, IMX882_AAC_EEPROM_I2C_ADDR);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			vendor_id = read_imx882_aac_eeprom_module(0x0001);
			if(vendor_id==0x10){
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_info("[%s] i2c write id: 0x%x, sensor id: 0x%x, vendor_id:0x%x\n",
					__func__, imgsensor.i2c_write_id, *sensor_id,vendor_id);
				read_sensor_Cali();
...
static kal_uint32 imx882_ana_gain_table[] = {
	100000,
	100098,
	100196,
	100294,
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(imx882_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)imx882_ana_gain_table,
			sizeof(imx882_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		imx882_get_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		 ...
		 *	(*feature_para_len));
		 */
		imx882_set_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
	...
	case SENSOR_FEATURE_SET_AWB_GAIN:
		...
		if (imgsensor.sensor_mode == IMGSENSOR_MODE_CUSTOM2 || imgsensor.sensor_mode == IMGSENSOR_MODE_CUSTOM3) {
			/*write AWB gain to sensor*/
			imx882_awb_gain((UINT32)*(feature_data_32 + 1),
...
UINT32 IMX882_AAC_MAIN_I_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
} /* IMX882_AAC_MAIN_I_MIPI_RAW_SensorInit */
