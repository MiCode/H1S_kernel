...
#include "imx882ofilmmipiraw_Sensor.h"
#include "imx882ofilm_eeprom.h"
#include "imx882ofilmmipiraw_Setting.h"

#undef VENDOR_EDIT

#define USE_BURST_MODE 1
#define QSC_LRC_DUMP 0
#define FPT_SEAMLESS_SUPPORT 1

/***************Modify Following Strings for Debug**********************/
#define PFX "imx882_ofilm_main_ii_camera_sensor"
/****************************   Modify end	**************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX882_OFILM_MAIN_II_SENSOR_ID,
	...
	.custom1 = {    
		.pclk = 877680000,//877680000,
		.linelength = 7500,//7500,
		.framelength = 4876, //3900,//4876,
		.startx = 0,
		.starty = 0,
		.grabwindow_width = 4096,
		.grabwindow_height = 3072,
	...
	.min_gain_fullsize = 64, /*1x gain*/
	.max_gain_fullsize = 1024, /*16x gain*/
	.min_gain_iso = 50,
	.margin = 56,		/* sensor framelength & shutter margin */
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] = {
	...
	{8192, 6144, 000, 768, 8192, 4608, 4096, 2304,	0000, 0000, 4096, 2304, 0, 0, 4096, 2304}, /* slim_video*/	//60fps@4096x2304 [slim] [pll:1332]	[piex:913.37]
	...
	{8192, 6144, 000, 000, 8192, 6144, 8192, 6144,	0000, 0000, 8192, 6144, 0, 0, 8192, 6144}, /* custom2 */	//10fps@8192x6144 [full] [pll:1278]	[piex:876.34]
	{8192, 6144,2048,1536, 4096, 3072, 4096, 3072,	0000, 0000, 4096, 3072, 0, 0, 4096, 3072}, /* custom3 */	//30fps@4096x3072 (crop from full size)	[piex:701.49]
};
...
static struct SENSOR_VC_INFO2_STRUCT SENSOR_VC_INFO2[6] = {
	...
	},
	{
		0x01, 0x0a, 0x00, 0x08, 0x40, 0x00,//hs_video
		{
			...
			{VC_PDAF_STATS, 0x00, 0x30, 0x960, 0x10E},
		},
		1
	},
	{
		0x01, 0x0a, 0x00, 0x08, 0x40, 0x00,//CUSTOM1_DUAL
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 0x1000, 0xc00},
			{VC_PDAF_STATS, 0x00, 0x30, 0x1400, 0x300},
		},
	...
	{
		0x01, 0x0a, 0x00, 0x08, 0x40, 0x00,//custom3 2Exp
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 0x1000, 0xc00},
			{VC_PDAF_STATS, 0x00, 0x30, 0xA00,  0x600},
...
static kal_uint16 imx882ofilm_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
static kal_uint16 imx882ofilm_table_write_cmos_sensor(kal_uint16 *para,
						 kal_uint32 len)
{
	char puSendCmd[I2C_BUFFER_LEN];
	kal_uint32 tosend, IDX;
...
static void imx882ofilm_get_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void imx882ofilm_set_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	imx882ofilm_table_write_cmos_sensor(regDa, regNum*2);
}
...
#define EEPROM_QSC_ADDR	0x2EFF
#define EEPROM_LRC_ADDR 0x3B04
#define SENEOR_QSC_ADDR 0xC000
#define SENEOR_LRC0_ADDR 0xD200
#define SENEOR_LRC1_ADDR 0xD300

static kal_uint16 imx882ofilm_QSC_setting[3072 * 2];
static kal_uint16 imx882ofilm_LRC_setting[384 * 2];


static void read_sensor_Cali(void)
{
	kal_uint16 idx = 0;
	kal_uint8 otp_data[8] 	= {0};
	kal_uint16 addr_qsc 	= EEPROM_QSC_ADDR;
	kal_uint16 sensor_qsc  	= SENEOR_QSC_ADDR;
	...
	for (idx = 0; idx < 4; idx++)
		otp_data[idx] = read_cmos_eeprom_8(0x2EFB + idx);//0x00,0x0c,0x01,0x02
	for (idx = 0; idx < 4; idx++)
		otp_data[idx + 4] = read_cmos_eeprom_8(0x3B00 + idx);//0x00,0x0d,0x01,0x02
#if QSC_LRC_DUMP
	for (idx = 0; idx < 4; idx++)
		LOG_INF("OTP data: [FLAG:%d]0x%x, 0x%x]\n",idx,(0x2EFB + idx),otp_data[idx]);
	for (idx = 0; idx < 4; idx++)
		LOG_INF("OTP data: [FLAG:%d]0x%x, 0x%x]\n",idx,(0x3B00 + idx),otp_data[idx+4]);
#endif
	if ((otp_data[0] == 0x00) && (otp_data[1] == 0x0C) && (otp_data[2] == 0x01) && (otp_data[3] == 0x02)){
		LOG_INF("OTP data: QSC date write!\n");
		otp_flag = OTP_QSC_CUSTOM;

		for (idx = 0; idx < 3072; idx++) {
			addr_qsc = EEPROM_QSC_ADDR + idx;
			sensor_qsc = SENEOR_QSC_ADDR + idx;
			imx882ofilm_QSC_setting[2 * idx] = sensor_qsc;
			imx882ofilm_QSC_setting[2 * idx + 1] = read_cmos_eeprom_8(addr_qsc);
		}
	} else {
		LOG_INF("OTP data: No QSC Data !\n");
	}


	if((otp_data[4] == 0x00) && (otp_data[5] == 0x0D) && (otp_data[6] == 0x01) && (otp_data[7] == 0x02)){
		LOG_INF("OTP data: LRC date write!\n");
		otp_flag = OTP_QSC_CUSTOM;
		for (idx = 0; idx < 192; idx++) {
			imx882ofilm_LRC_setting[2 * idx] = sensor_lrc0 + idx;
			imx882ofilm_LRC_setting[2 * idx + 1] = read_cmos_eeprom_8(addr_lrc + idx);
			imx882ofilm_LRC_setting[2 * idx + 192 * 2] = sensor_lrc1 + idx;
			imx882ofilm_LRC_setting[2 * idx + 1 + 192 * 2] = read_cmos_eeprom_8(addr_lrc + 192 + idx);
		}
	} else {
...
static void write_sensor_QSC(void)
{
	...
	if ((otp_flag == OTP_QSC_CUSTOM) || (otp_flag == OTP_QSC_INTERNAL)) {
		imx882ofilm_table_write_cmos_sensor(imx882ofilm_QSC_setting,sizeof(imx882ofilm_QSC_setting) / sizeof(kal_uint16));
		imx882ofilm_table_write_cmos_sensor(imx882ofilm_LRC_setting,sizeof(imx882ofilm_LRC_setting) / sizeof(kal_uint16));
	}
	...
	LOG_INF("OTP data:======[ERRPROM_ADDR, EERPROM_VAULE; SENEOR_ADDR, SENEOR_VAULE][QSC]======\n");
	for (idx = 0; idx < 4; idx++)
		LOG_INF("OTP data: [FLAG:%d][0x%x, 0x%x]\n",idx,(0x2EFB + idx),read_cmos_eeprom_8(0x2EFB + idx));
	for (idx = 0; idx < 3072; idx++)
	{
		addr_qsc = EEPROM_QSC_ADDR + idx;
		sensor_qsc = SENEOR_QSC_ADDR + idx;
		LOG_INF("OTP data: [QSC:%d][0x%x, 0x%x; 0x%x, 0x%x]\n",idx,addr_qsc,read_cmos_eeprom_8(addr_qsc),sensor_qsc,read_cmos_sensor_8(sensor_qsc));
	}

	LOG_INF("OTP data:======[ERRPROM_ADDR, EERPROM_VAULE; SENEOR_ADDR, SENEOR_VAULE][LRC]======\n");
	for (idx = 0; idx < 4; idx++)
		LOG_INF("OTP data: [FLAG:%d][0x%x, 0x%x]\n",idx,(0x3B00 + idx),read_cmos_eeprom_8(0x3B00 + idx));
	for (idx = 0; idx < 192; idx++)
	{
...
static void write_shutter(kal_uint32 shutter, kal_bool gph)
{
	...
	if (shutter >
		...
		write_cmos_sensor_8(0x3160,
			read_cmos_sensor(0x3160) | (l_shift & 0xf));
...
#define IMX882OFILM_GAIN_TABL_SIZE 432
kal_uint16 imx882ofilm_gain_ratio[IMX882OFILM_GAIN_TABL_SIZE] = {
	64,
	65,
	66,
	67,
...
kal_uint16 imx882ofilm_gain_reg[IMX882OFILM_GAIN_TABL_SIZE] = {
	0,
	240,
	496,
	720,
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	{
		if (gain_value < imgsensor_info.min_gain_fullsize || gain_value > imgsensor_info.max_gain_fullsize) {
			LOG_INF("Error: gain value out of range %d [%d]", gain,__LINE__);

			if (gain_value < imgsensor_info.min_gain_fullsize)
				gain_value = imgsensor_info.min_gain_fullsize;
			else if (gain_value > imgsensor_info.max_gain_fullsize)
				gain_value = imgsensor_info.max_gain_fullsize;
		}
	}else{
...
#ifdef USE_GAIN_TABLE
	reg_gain = imx882ofilm_gain_reg[IMX882OFILM_GAIN_TABL_SIZE - 1];
	for (i = 0; i < IMX882OFILM_GAIN_TABL_SIZE; i++) {
		if (gain_value <= imx882ofilm_gain_ratio[i]) {
			reg_gain = imx882ofilm_gain_reg[i];
			break;
		}
	}
#else
...
static kal_uint16 set_gain_w_gph(kal_uint16 gain, kal_bool gph)
{
	...
	if (imgsensor.sensor_mode == IMGSENSOR_MODE_CUSTOM2 || imgsensor.sensor_mode == IMGSENSOR_MODE_CUSTOM3)
	{
		if (gain < imgsensor_info.min_gain_fullsize || gain > imgsensor_info.max_gain_fullsize) {
			LOG_INF("Error: gain value %d [%d]", gain,__LINE__);

			if (gain < imgsensor_info.min_gain_fullsize)
				gain = imgsensor_info.min_gain_fullsize;
			else
				gain = imgsensor_info.max_gain_fullsize;
		}
	}else{
 ...
 *
 *************************************************************************/
static kal_uint16 imx882ofilm_feedback_awbgain[] = {
	0x0b90, 0x00,
	0x0b91, 0x01,
	0x0b92, 0x00,
	0x0b93, 0x01,
...
static void imx882ofilm_awb_gain(kal_uint32 r_gain, kal_uint32 b_gain)
{
	UINT32 r_gain_int = 0;
	UINT32 b_gain_int = 0;
	...
	imx882ofilm_feedback_awbgain[1] = r_gain_int;
	imx882ofilm_feedback_awbgain[3] = (
		((r_gain*100) / 512) - (r_gain_int * 100)) * 2;
	imx882ofilm_feedback_awbgain[5] = b_gain_int;
	imx882ofilm_feedback_awbgain[7] = (
		((b_gain * 100) / 512) - (b_gain_int * 100)) * 2;
	imx882ofilm_table_write_cmos_sensor(imx882ofilm_feedback_awbgain,
		sizeof(imx882ofilm_feedback_awbgain)/sizeof(kal_uint16));
	PK_PR_ERR("[%s][0x%x,0x%x,0x%x,0x%x]\n",__func__,read_cmos_sensor_8(0x0b90),read_cmos_sensor_8(0x0b91),read_cmos_sensor_8(0x0b92),read_cmos_sensor_8(0x0b93));
}
...
{
	LOG_INF("E\n");
	imx882ofilm_table_write_cmos_sensor(imx882ofilm_init_setting,
		sizeof(imx882ofilm_init_setting)/sizeof(kal_uint16));
...
static void preview_setting(void)
{
	...
	imx882ofilm_table_write_cmos_sensor(imx882ofilm_preview_setting,
		sizeof(imx882ofilm_preview_setting)/sizeof(kal_uint16));
...
static void capture_setting(kal_uint16 currefps)
{
	...
	imx882ofilm_table_write_cmos_sensor(imx882ofilm_capture_30_setting,
		sizeof(imx882ofilm_capture_30_setting)/sizeof(kal_uint16));
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	imx882ofilm_table_write_cmos_sensor(imx882ofilm_normal_video_setting,
	sizeof(imx882ofilm_normal_video_setting)/sizeof(kal_uint16));
...
static void hs_video_setting(void)
{
	...
	LOG_INF("%s E! \n", __func__);
	imx882ofilm_table_write_cmos_sensor(imx882ofilm_hs_video_setting,
		sizeof(imx882ofilm_hs_video_setting)/sizeof(kal_uint16));
}
...
static kal_uint32 seamless_switch(enum MSDK_SCENARIO_ID_ENUM scenario_id, uint32_t *ae_ctrl)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	{
		...
		FMC_GPH_START;
		imx882ofilm_table_write_cmos_sensor(addr_data_pair_fmc_pre,
	...
	case MSDK_SCENARIO_ID_CUSTOM3:
	{
		...
		FMC_GPH_START;
		imx882ofilm_table_write_cmos_sensor(addr_data_pair_fmc_cus3,
				sizeof(addr_data_pair_fmc_cus3) / sizeof(kal_uint16));
...
{
	LOG_INF("%s E\n", __func__);
	imx882ofilm_table_write_cmos_sensor(imx882ofilm_slim_video_setting,
		sizeof(imx882ofilm_slim_video_setting)/sizeof(kal_uint16));
}
...
static void custom1_setting(void)
{
	...
	imx882ofilm_table_write_cmos_sensor(imx882ofilm_custom1_setting,
		sizeof(imx882ofilm_custom1_setting)/sizeof(kal_uint16));
	LOG_INF("X");
}

//10fps@8192x6144 [full]
static void custom2_setting(void)
{
	LOG_INF("%s 10 fps E! currefps\n", __func__);
	/*************MIPI output setting************/

	imx882ofilm_table_write_cmos_sensor(imx882ofilm_custom2_setting,
		sizeof(imx882ofilm_custom2_setting)/sizeof(kal_uint16));

	LOG_INF("X");
}

//30fps@4096x3072 (crop from full size, insensorzoom)
static void custom3_setting(void)
{
	LOG_INF("%s 30 fps E! currefps\n", __func__);
	/*************MIPI output setting************/

	imx882ofilm_table_write_cmos_sensor(imx882ofilm_custom3_setting,
		sizeof(imx882ofilm_custom3_setting)/sizeof(kal_uint16));

	LOG_INF("X");
}

#define IMX882_OFILM_EEPROM_I2C_ADDR 0xA2
static kal_uint16 read_imx882ofilm_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, IMX882_OFILM_EEPROM_I2C_ADDR);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			vendor_id = read_imx882ofilm_eeprom_module(0x0001);
			if(vendor_id==0x07){
			    	*sensor_id = ((read_cmos_sensor_8(0x0016) << 8) | read_cmos_sensor_8(0x0017)) + 1;
			}
			else{
			   	LOG_INF("check id fail i2c 0x%x, vendor_id:0x%x, sensor_id: 0x%x\n",imgsensor.i2c_write_id,vendor_id,*sensor_id);
			...
			LOG_INF("sensor_id = 0x%x, imgsensor_info.sensor_id = 0x%x,vendor_id:0x%x\n",
				*sensor_id, imgsensor_info.sensor_id,vendor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 imx882ofilm_ana_gain_table[] = {
	100000,
	100098,
	100196,
	100294,
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(imx882ofilm_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)imx882ofilm_ana_gain_table,
			sizeof(imx882ofilm_ana_gain_table));
		}
		break;
	case SENSOR_FEATURE_GET_GAIN_RANGE_BY_SCENARIO:
		if (imgsensor.sensor_mode == IMGSENSOR_MODE_CUSTOM2 || imgsensor.sensor_mode == IMGSENSOR_MODE_CUSTOM3)
		{
			*(feature_data + 1) = imgsensor_info.min_gain_fullsize;
	...
	case SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO:
		...
		*(feature_data + 2) = 6; // 2/3 exp do not have to round up
		break;
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 2154300;
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		switch (*feature_data) {
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:	//30fps@4096x3072
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		case MSDK_SCENARIO_ID_CUSTOM1:
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:		//30fps@4096x2304
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:		//120fps@1920x1080
		case MSDK_SCENARIO_ID_CUSTOM3:			//30fps@4096x3072
	...
	case SENSOR_FEATURE_GET_STAGGER_MAX_EXP_TIME:
		if (*feature_data == MSDK_SCENARIO_ID_CUSTOM2
			...
			// see IMX882OFILM SRM, table 5-22 constraints of COARSE_INTEG_TIME
			switch (*(feature_data + 1)) {
			case VC_STAGGER_NE:
			case VC_STAGGER_ME:
			case VC_STAGGER_SE:
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		imx882ofilm_get_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		 ...
		 *	(*feature_para_len));
		 */
		imx882ofilm_set_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
	...
	case SENSOR_FEATURE_SET_AWB_GAIN:
		...
		if (imgsensor.sensor_mode == IMGSENSOR_MODE_CUSTOM2 || imgsensor.sensor_mode == IMGSENSOR_MODE_CUSTOM3) {
			/*write AWB gain to sensor*/
			imx882ofilm_awb_gain((UINT32)*(feature_data_32 + 1),
...
UINT32 IMX882_OFILM_MAIN_II_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
} /* IMX882_OFILM_MAIN_II_MIPI_RAW_SensorInit */
