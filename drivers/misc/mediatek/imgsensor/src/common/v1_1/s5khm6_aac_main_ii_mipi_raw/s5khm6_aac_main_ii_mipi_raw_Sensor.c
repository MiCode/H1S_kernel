...
#include "s5khm6_aac_main_ii_mipi_raw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KHM6_AAC_MAIN_II_SENSOR_ID,	//0x1AD6+1
...
static	unsigned int sensor_debug_burst_ii = 1;

module_param(sensor_debug_burst_ii, uint, 0644);
MODULE_PARM_DESC(sensor_debug_burst_ii, "imgsensor_debug");

unsigned int *sensor_debug_burst_flag_ii = &sensor_debug_burst_ii;
...
{
	LOG_INF("+, burst: %d", *sensor_debug_burst_flag_ii);
	write_cmos_sensor(0xFCFC, 0x4000);
	...
	mdelay(1);
	// 2nd Tnp
	if (*sensor_debug_burst_flag_ii == 1)
	{
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_INF("s5khm6 aac sensor version number = 0x%x, vendor id = 0x%x",sensor_version, vendor_id);
...
UINT32 S5KHM6_AAC_MAIN_II_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
