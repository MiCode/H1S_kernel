...
#include "s5khm6_ofilm_main_i_mipi_raw_Sensor.h"
#include "s5khm6_ofilm_setting.h"
...
#undef VENDOR_EDIT
#define PFX "s5khm6_ofilm_main_i"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KHM6_OFILM_MAIN_I_SENSOR_ID,	//0x1AD6
	...
	.max_gain_fullsize = 8 * BASEGAIN, /*8x gain*/
	.min_gain_iso = 50,
	.exp_step = 2,
    	.gain_step = 2,
    	.gain_type = 2,
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[6] = {
	...
	{0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 ...
	 0x01, 0x2b, 0x07D0, 0x05DC, 0x00, 0x00, 0x0000, 0x0000},
    ...
    {0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
     ...
     0x01, 0x2B, 0x0538, 0x03E8, 0x03, 0x00, 0x0000, 0x0000},
};
/* If mirror flip */
...
static	unsigned int sensor_debug_burst = 1;

module_param(sensor_debug_burst, uint, 0644);
MODULE_PARM_DESC(sensor_debug_burst, "imgsensor_debug");

unsigned int *sensor_debug_burst_flag = &sensor_debug_burst;
...
static kal_uint16 table_burst_write_cmos_sensor(
...
{
	LOG_INF("+, burst: %d", *sensor_debug_burst_flag);
	write_cmos_sensor(0xFCFC, 0x4000);
	...
	mdelay(1);
	// 2nd Tnp
	if (*sensor_debug_burst_flag == 1)
	{
		table_burst_write_cmos_sensor(s5khm6_init_setting,	sizeof(s5khm6_init_setting)/sizeof(kal_uint16));
	} else {
...
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(0x21 >> 8), (char)(0x21 & 0xFF) };
	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, 0xA2);
	return get_byte;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	kal_uint16 sensor_version = 0;
	vendor_id = get_vendor_id();
	sensor_version = get_sensor_version();//sensor_version sf:0x2 gf:0x1
	LOG_INF("s5khm6 ofilm sensor version number = 0x%x, vendor id = 0x%x",sensor_version, vendor_id);
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id = read_cmos_sensor(0x0000);
			LOG_INF("s5khm6-ofilm sensor id = 0x%x, vendor id = 0x%x",*sensor_id ,vendor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("s5khm6-ofilm i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				get_custom_info(&addr_info, &custom_info);
				return ERROR_NONE;
			}
			else{
				LOG_INF("s5khm6-ofilm read vendor id fail");
				*sensor_id = 0xFFFFFFFF;
				return ERROR_SENSOR_CONNECT_FAIL;
			}
			}
			LOG_INF("s5khm6-ofilm Read sensor id fail, sensor id: 0x%x, i2c: 0x%x\n", *sensor_id, imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_GAIN_RANGE_BY_SCENARIO:
		*(feature_data + 1) = imgsensor_info.min_gain;
		...
		//LOG_INF("SENSOR_FEATURE_GET_GAIN_RANGE_BY_SCENARIO:max_gain: %d\n", *(feature_data + 2));
		break;
...
UINT32 S5KHM6_OFILM_MAIN_I_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
