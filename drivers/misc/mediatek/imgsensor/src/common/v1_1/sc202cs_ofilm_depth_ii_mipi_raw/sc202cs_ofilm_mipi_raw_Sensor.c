 ...
 * Filename:
 * ---------
 *     sc202cs_ofilm_mipi_raw_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "sc202cs_ofilm_mipi_raw_Sensor.h"
#include "imgsensor_custom.h"

/* O17 code  by yumeng at 2024/04/09 start */
#define Lens_ID 0xB7
/* O17 code  by yumeng at 2024/04/09 end */
/****************************Modify Following Strings for Debug****************************/
#define PFX "sc202cs_ofilm_mipi_raw"
...
static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = SC202CS_OFILM_DEPTH_II_SENSOR_ID,
	...
	#if 1
    .custom1  = {
		.pclk = 72000000,//72000000,
		.linelength = 1920,//2400,
		.framelength = 1562,
		.startx = 0,
		.starty = 0,
		.grabwindow_width = 1600,
	...
	.min_gain_iso = 50,
	.gain_step = 2,
	.gain_type = 2,/*to be modify,no gain table for gc*/
...
{
	return ((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108)) + 3; //0xeb52 -> 0xeb55
}
...
#define SC202CS_SENSOR_GAIN_MAX              16384 //(16 * SC202CS_SENSOR_GAIN_BASE)
...
{
	LOG_INF("depth-ofilm sensor_init");
...
/* O17 code by yumeng at 2024/04/09 start */
     /* O17 code by yumeng at 2024/04/09 start */
  static kal_int8 sc202cs_ofilm_read_general_info(int *addr, kal_uint16 addr_len, kal_uint8 *result)
     /* O17 code by yumeng at 2024/04/09 end */
  {
  	int i = 0;
    ...
    write_cmos_sensor(0x440b, 0x1f);
  	write_cmos_sensor(0x0100, 0x01);
  	write_cmos_sensor(0x4400, 0x11);

  	for (i = 0; i < addr_len; i++) {
  		result[i] = read_cmos_sensor(addr[i]);
        LOG_INF("depth-ofilm read result[%d] is %x", i, result[i]);
  	}
  	return 0;
  }
   /* O17 code by yumeng at 2024/04/09 start */
  unsigned int sc202cs_ofilm_read_otp(void)
  {
  	int ret = 0;
	kal_uint8 i;
/* O17 code by yumeng at 2024/04/09 start */ 
...
/* O17 code by yumeng at 2024/04/09 end */
    ret = sc202cs_ofilm_read_general_info(otpdata_addr, ARRAY_SIZE(otpdata_addr), otpData);
    /* O17 code by yumeng at 2024/04/09 end */
  	if (ret != 0) {
  		LOG_INF("depth-ofilm read fuseid failed\n");
  		return -1;
  	}
     for(i = 0; i < ARRAY_SIZE(otpData); i++){
		LOG_INF("depth-ofilm read otp data 0x%x", otpData[i]);
	}
	for(i = 0; i < custom_info.fuse_id_len; i++){
        u16 fuse = otpData[15+i];
        sprintf(custom_info.fuse_id + i*2, "%02x", fuse);
    }
    for(; i < 17; i++){
        strncpy(custom_info.fuse_id + i * 2, "00", 2);
	}
    sprintf(custom_info.sn_info, "%014d", 0);
    custom_info.vendor_id = otpData[1];
    custom_info.lens_id = otpData[2];
    custom_info.sensor_id = otpData[3];
  	return ret;
  }

  unsigned int sc202cs_ofilm_read_otp_info(struct i2c_client *client,
  		 unsigned int addr, unsigned char *data, unsigned int size)
  {
  	int num = ARRAY_SIZE(otpData);
	int index = 0;
	LOG_INF(" depth-ofilm addr:0x%x ,size:%d",addr, size);
	for(;index<num ;index++){
   ...
   /* O17 code by yumeng at 2024/04/09 start */
    EXPORT_SYMBOL(sc202cs_ofilm_read_otp_info);
   /* O17 code by yumeng at 2024/04/09 end */
...
/* O17 code  by yumeng at 2024/04/09 start */
	...
	vendor_id = read_cmos_sensor(0x8009);
	LOG_INF("depth-ofilm vendor_id = 0x%x",vendor_id);
	LOG_INF("depth-ofilm getimgsensorid!!!\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		LOG_INF("depth-ofilm return_sensor_id:0x%x , lens_id = 0x%x\n",*sensor_id,lens_id);
	    }
	    else{
	   	LOG_INF("check id fail i2c 0x%x, sensor_id: 0x%x\n",
  						imgsensor.i2c_write_id,
  						*sensor_id);
 						*sensor_id = 0xFFFFFFFF;
	    }
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("depth-ofilm i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                if(lens_id == Lens_ID){
                    ret = sc202cs_ofilm_read_otp();
                    return ERROR_NONE;
                }
                else{
                    LOG_INF("depth-ofilm read lens_id fail");
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                }
            }
/* O17 code  by yumeng at 2024/04/09 end */
            LOG_INF("depth-ofilm Read sensor id fail, return_sensor_id: 0x%x\n",*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
UINT32 SC202CS_OFILM_DEPTH_II_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SC202CS_OFILM_MIPI_RAW_SensorInit    */
