...
int mtk_power_pd_mode_hiz(struct pd_port *pd_port, bool en);
...
static uint8_t pd_dpm_build_rdo(struct pd_port *pd_port)
{
	...
	if(req_info.oper_ma == 0) {
		tcpci_sink_vbus(
			tcpc, TCP_VBUS_CTRL_REQUEST, TCPC_VBUS_SINK_5V, 0);
		mtk_power_pd_mode_hiz(pd_port, true);
	}
...
int mtk_power_pd_mode_hiz(struct pd_port *pd_port, bool en)
{
	...
	if (IS_ERR_OR_NULL(pd_port->chg_dev))
		pd_port->chg_dev = get_charger_by_name("primary_chg");

	cancel_delayed_work(&pd_port->mtk_power_work);

	if (en) {
		pr_info("mtk-cc: enter charger_dev_enable_hz\n");
		charger_dev_set_mivr(pd_port->chg_dev, 6500000);
		charger_dev_enable_hz(pd_port->chg_dev, 1);
		schedule_delayed_work(
				&pd_port->mtk_power_work, msecs_to_jiffies(300));
	}

	return 0;
}

static void mtk_power_delay_work(struct work_struct *work)
{
	struct pd_port *pd_port = container_of(work, struct pd_port,
					       mtk_power_work.work);

	pr_info("mtk-cc: exit charger_dev_enable_hz\n");
	charger_dev_set_mivr(pd_port->chg_dev, 4600000);
	charger_dev_enable_hz(pd_port->chg_dev, 0);
	return;
}
...
int pd_dpm_core_init(struct pd_port *pd_port)
{
	...
	INIT_DELAYED_WORK(&pd_port->mtk_power_work, mtk_power_delay_work);
