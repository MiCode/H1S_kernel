...
static unsigned long power_debug_enable = 0;
...
static struct regmap *spmi_regmap = NULL;
...
void pm_system_dbg_info_print(enum system_dbg_type type)
{
	...
	POWER_DEBUG("power-debug:  pm_system_dbg_info_print : type: %d",type);
...
#else
extern void debug_pintctrl_msm_enable(void);
extern void debug_pintctrl_msm_disable(void);
void mi_power_regmap_register(struct regmap *regmap)
{
	spmi_regmap = regmap;
}
EXPORT_SYMBOL(mi_power_regmap_register);

void spmi_vol_enter_dump_sec(bool enable)
{
	if (!spmi_regmap)
	{
		pr_info("regmap 0-00 can not register\n");
		return;
	}
	...
	if (enable)
	{
		...
		//config register for vol- trigger ramdump
		regmap_write(spmi_regmap, 0x1316, 0x42);
		regmap_write(spmi_regmap, 0x0811, 0x12);
		regmap_write(spmi_regmap, 0x0813, 0x12);
		regmap_write(spmi_regmap, 0x0815, 0x02);
		regmap_write(spmi_regmap, 0x0844, 0x00);
		regmap_write(spmi_regmap, 0x0845, 0x00);
		regmap_write(spmi_regmap, 0x0846, 0x01);
		regmap_write(spmi_regmap, 0x0847, 0x80);
	}
}

/*   bit 0: 0-skip gpio, 1-print gpio in suspend
 *   bit 1: 0-skip regulator, 1-print regulator in suspend
 *   bit 24: config vol- to trigger ramdump, high 8 bit for secret code
 *   */
static ssize_t power_debug_store(struct class *cls, struct class_attribute *attr, const char *buf, size_t count)
{
	unsigned long val;
	...
	if (val != 0)
		power_debug_enable = (val & 0xffffffff) | power_debug_enable;
	else
		power_debug_enable = 0;

	//low 28bit for function
	if (power_debug_enable & 0x01)
		debug_pintctrl_msm_enable();
	else
		debug_pintctrl_msm_disable();

	//high 8bit for secert code ctrol
	if (power_debug_enable & 0x01000000)
		spmi_vol_enter_dump_sec(true);
...
static ssize_t power_debug_show (struct class *cls, struct class_attribute *attr, char *buf)
{
	...
	enable = scnprintf(buf, PAGE_SIZE, "gpio suspend print:%s, ", (power_debug_enable & 0x1)?"enable":"disable");
	enable += scnprintf(buf+enable, PAGE_SIZE, "regulator suspend print:%s\n", (power_debug_enable & 0x2)?"enable":"disable");
...
static int(*mi_cx_stats_get_ss_vote_info)(int, struct qcom_stats_cx_vote_info*) = NULL;

void register_mi_cx_stats_get_ss_vote_info(int(*cxvt)(int, struct qcom_stats_cx_vote_info*))
{
	mi_cx_stats_get_ss_vote_info = cxvt;
}
EXPORT_SYMBOL(register_mi_cx_stats_get_ss_vote_info);

int cxvt_stats_sysfs_print(char *buf)
{
	...
	if(mi_cx_stats_get_ss_vote_info==NULL)
	{
		POWER_ERR("mi_cx_stats_get_ss_vote_info is NULL!");
		return 0;
	}

	memset(vote_info, 0, (int)sizeof(struct qcom_stats_cx_vote_info)*MAX_DRV);

	mi_cx_stats_get_ss_vote_info(MAX_DRV, vote_info);

	for(i = 0; i < MAX_DRV; i++){
		cnt += sprintf(buf+cnt, "owen DRV: %d, vote: %d \n", i, vote_info[i].level);
	}

	return cnt;
}

static ssize_t cxvt_state_show (struct class *cls, struct class_attribute *attr, char *buf)
{
	return cxvt_stats_sysfs_print(buf);
}

static int(*mi_ddr_stats_get_ss_vote_info)(int, struct ddr_stats_ss_vote_info*) = NULL;

void register_mi_ddr_stats_get_ss_vote_info(int(*ddrvt)(int, struct ddr_stats_ss_vote_info*))
{
	mi_ddr_stats_get_ss_vote_info = ddrvt;
}
EXPORT_SYMBOL(register_mi_ddr_stats_get_ss_vote_info);

static ssize_t ddrvt_state_show(struct class *cls, struct class_attribute *attr, char *buf)
{
	struct ddr_stats_ss_vote_info vote_info[MAX_DRV];
	int i;
	int cnt= 0;

	if (mi_ddr_stats_get_ss_vote_info == NULL)
	{
		POWER_ERR("mi_ddr_stats_get_ss_vote_info is NULL!");
		return 0;
	}

	memset(vote_info, 0, (int)sizeof(struct ddr_stats_ss_vote_info) * MAX_DRV);

	mi_ddr_stats_get_ss_vote_info(MAX_DRV, vote_info);

	for(i = 0; i < MAX_DRV; i++){
		cnt += sprintf(buf+cnt, "owner DRV: %d avg: %d peak: %d\n",
...
static struct class_attribute power_debug_attrs[] = {
	...
	__ATTR(cxvt_state, 0664, cxvt_state_show, NULL),
	__ATTR(ddrvt_state, 0664, ddrvt_state_show, NULL),
	__ATTR_NULL
};
...
struct sleep_stats_pre {
	const char *name;
	u32 count;
	u64 accumulated;
};

static struct stats_drvdata *ss_data = NULL;
...
static struct sleep_stats_pre sleep_info_pre[] = {
	{ "apss", 0, 0 },
	{ "modem", 0, 0 },
	{ "wpss", 0, 0 },
	{ "adsp", 0, 0 },
...
int subsystem_sleep_stats_sysfs_print(char *buf)
{
	...
	for (i = 0; i < ARRAY_SIZE(subsystems); i++) {
		if (subsystems[i].pid == SUBSYSTEM_STATS_OTHERS_NUM) {
			...
			if (NULL != ss_data && NULL != stat && NULL != ss_data->d[idx].base)
				memcpy_fromio(stat, ss_data->d[idx].base, sizeof(*stat));
			else {
				stat = NULL;
...
static void soc_rpmh_master_print_one(struct sleep_stats *stat,const char *name, struct sleep_stats_pre *stat_pre)
{
	u64 accumulated = stat->accumulated;
	char stat_type[5] = {0};
	memcpy(stat_type, &stat->stat_type, sizeof(u32));
	...
	if (stat->last_entered_at > stat->last_exited_at)
		accumulated += arch_timer_read_counter()
			- stat->last_entered_at;
	POWER_INFO("%s:%u:%llu sleep increased:%u:%llu\n",
		name, stat->count, accumulated, stat->count - stat_pre->count, accumulated - stat_pre->accumulated);
	if(stat->count - stat_pre->count > 5000){
		POWER_DEBUG("%s maybe wakeup(%u) too often\n", name, stat->count - stat_pre->count);
	}
	if(accumulated - stat_pre->accumulated == 0){
		POWER_DEBUG("%s sleep is not well\n", name);
	}
	stat_pre->count = stat->count;
	stat_pre->accumulated = accumulated;
	POWER_DEBUG("Last Entered/Exit:%llu/%llu\n", stat->last_entered_at, stat->last_exited_at);
}
	...
	for (i = 0; i < ARRAY_SIZE(subsystems); i++) {
		...
		if (!IS_ERR_OR_NULL(stat))
				soc_rpmh_master_print_one(stat,
						subsystems[i].name,  &sleep_info_pre[i]);
		else
			POWER_DEBUG("subsystem %s stats %s.\n", subsystems[i].name, IS_ERR(stat)?"is not support":"is null");
	}
...
void subsystem_sleep_stats_dbg_register(struct stats_drvdata *prv_data)
{
	if (prv_data == NULL)
		return;
...
static struct delayed_work batt_current_print;
#define BATT_CURRENT_PRINT_PERIOD_S 30

static void pm_get_batt_current_now(void){
	struct power_supply *psy_bat = NULL;
	union power_supply_propval prop = {0,};
	int rc = -1;

	psy_bat = power_supply_get_by_name("battery");
	if(!psy_bat){
		POWER_ERR("get battery power supply failed\n");
		return;
	}

	rc = power_supply_get_property(psy_bat, POWER_SUPPLY_PROP_CURRENT_NOW,
		&prop);
	if(rc < 0){
		POWER_ERR("get battery current failed: %d\n", rc);
	}else{
		POWER_INFO("battery current: %dmA \n", prop.intval/1000);
	}
}

static void batt_current_print_func(struct work_struct *work){
	pm_get_batt_current_now();
	schedule_delayed_work(&batt_current_print,
		round_jiffies_relative(msecs_to_jiffies(BATT_CURRENT_PRINT_PERIOD_S*1000)));
}

extern void clock_debug_print_enabled_clocks(struct seq_file *s);
// MIUI ADD: Power_LogEnhance
extern void regulator_debug_print_enable_regulators(void);
// END Power_LogEnhance
static void power_debug_suspend_trace_probe(void *unused,const char *action, int val, bool start)
{
	...
	if (val > 0 && !strcmp("machine_suspend", action)) {
		POWER_DEBUG("soc_lpm debug infos :\n");
		XIAOMI_POWER_CST_PRINT(TAGS);

		soc_lpm_suspend_info();

		if (start) {
			clock_debug_print_enabled_clocks(NULL);
			if (power_debug_enable & 0x02)
				regulator_debug_print_enable_regulators();
		}
	}
}
...
static void power_debug_freeze_todo_trace_probe(void *unused,unsigned int todo, unsigned int elapsed_msecs, bool wq_busy)
{
...
static ssize_t ddr_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (NULL != ss_data && NULL != stat && NULL != ss_data->d[DDR].base) {
        memcpy_fromio(stat, ss_data->d[DDR].base, sizeof(*stat));
    } else
        POWER_ERR("ss_data is null.");
...
static ssize_t cxsd_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (NULL != ss_data && NULL != ss_data->d[CXSD].base)
        memcpy_fromio(stat, ss_data->d[CXSD].base, sizeof(*stat));
    else
        POWER_ERR("ss_data is null.");
...
static ssize_t aosd_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (NULL != ss_data && NULL != ss_data->d[AOSD].base)
        memcpy_fromio(stat, ss_data->d[AOSD].base, sizeof(*stat));
    else
        POWER_ERR("ss_data is null.");
...
#else
extern int clk_debug_suspend_atomic_enable(u64 val);
static int __init pm_debug_init(void)
{
	int ret = 0;
	int i;
   ...
   clk_debug_suspend_atomic_enable(true);
   register_trace_suspend_resume(power_debug_suspend_trace_probe, NULL);
   register_trace_android_vh_try_to_freeze_todo(power_debug_freeze_todo_trace_probe, NULL);
	...
	INIT_DELAYED_WORK(&batt_current_print, batt_current_print_func);
	schedule_delayed_work(&batt_current_print,
		round_jiffies_relative(msecs_to_jiffies(BATT_CURRENT_PRINT_PERIOD_S*1000)));
...
static void __exit pm_debug_exit(void)
{
	...
	clk_debug_suspend_atomic_enable(false);
	ret = unregister_trace_suspend_resume(power_debug_suspend_trace_probe, NULL);
	ret = unregister_trace_android_vh_try_to_freeze_todo(power_debug_freeze_todo_trace_probe, NULL);
}
