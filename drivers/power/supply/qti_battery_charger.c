...
enum xm_property_id {
	...
	XM_PROP_QBG_CURR,
#if defined(CONFIG_MI_O16U_EUPD) || defined(CONFIG_MI_O11_EMPTY_BATTERY) || defined(CONFIG_MI_EU_REGION_OPEN_POWER)
	/*Support 100W EEA Chg Func*/
	XM_PROP_IS_EU_MODEL,
	XM_PROP_PPS_PTF,
#endif
...
static void xm_handle_adsp_sicmode_upload(struct battery_chg_dev *bcdev, struct chg_prop_upload_msg *Context);
				...
				size_t len)
{
	...
	struct chg_prop_upload_msg *chg_prop_upload_data = data;
	...
	case BC_XM_STATUS_UPLOAD:
		if (len == sizeof(*chg_prop_upload_data)) {
			xm_handle_adsp_sicmode_upload(bcdev, chg_prop_upload_data);
			ack_set = true;
			break;
		}
		break;
...
static void xm_handle_adsp_sicmode_upload(struct battery_chg_dev *bcdev, struct chg_prop_upload_msg *Context)
{
	...
	if (Context->property_id != XM_PROP_SMART_SIC_MODE) {
		return;
	}

	if (Context->upload) {
		snprintf(uevent_buf, MAX_STR_LEN, "POWER_SUPPLY_SMART_SIC_MODE=%d", Context->property_value);
