...
{
    dev_err(sc->dev,"%s:%d", __func__, en);
...
static int mtk_sc8541_enable_cp_adc(struct charger_device *chg_dev, bool enable)
{
    struct sc8541_chip *sc = charger_get_data(chg_dev);

    return sc8541_enable_adc(sc, enable);
}
...
static const struct charger_ops sc8541_chg_ops = {
    ...
    .enable_cp_adc = mtk_sc8541_enable_cp_adc,
};
#endif /*CONFIG_MTK_CLASS*/
                    ...
                    const struct i2c_device_id *id)
{
    ...
    ret = sc8541_set_work_mode(sc, *(int *)match->data);
    if (ret) {
    ...
    ret = sc8541_enable_adc(sc, false);
    if (ret) {
        dev_err(sc->dev,"Fail to disable adc!\n");
        goto err_set_mode;
    }
