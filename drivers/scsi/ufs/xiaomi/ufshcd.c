<<<<<<< HEAD   (aa6c53 Merge "o17v:slove smart_chg 81% stop charge" into lc-release)
...
#if defined(CONFIG_SCSI_UFS_XIAOMI_CLD)
	ufscld_on_idle(hba);
#endif
}
...
static void ufshcd_tune_unipro_params(struct ufs_hba *hba)
{
	...
	ret = ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXHSG3SYNCLENGTH), 0x4f);
	if (ret)
		pr_info("@debug %s %d dme set G3 fail:%d\n",
                      __func__, __LINE__, ret);
	ret = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TXHSG3SYNCLENGTH), 0x4f);
	if (ret)
		pr_info("@debug %s %d dme set G3 peer fail:%d\n",
                      __func__, __LINE__, ret);
	ret = ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXHSG1SYNCLENGTH), 0x4f);
	if (ret)
		pr_info("@debug %s %d dme set G1 fail:%d\n",
                      __func__, __LINE__, ret);

	ret = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TXHSG1SYNCLENGTH), 0x4f);
	if (ret)
		pr_info("@debug %s %d dme set G1 peer fail:%d\n",
                      __func__, __LINE__, ret);
}
...
#if defined(CONFIG_SCSI_UFS_XIAOMI_CLD)
	ufscld_init(hba); // we need use inquriy. move cld init after add_lus.
#endif
...
#if defined(CONFIG_SCSI_UFS_XIAOMI_CLD)
	if (ret && ufscld_get_state(hba) == CLD_SUSPEND)
		ufscld_resume(hba);
#endif
...
=======
>>>>>>> CHANGE (419d51 O17:memory:opensource)
