...
u32 optee_do_call_with_arg(struct tee_context *ctx, phys_addr_t parg)
{
	...
	if(DEBUG_CALL) pr_err("do call with args\n");
	...
	while (true) {
		...
		if(DEBUG_CALL) pr_err("invoke result: %08x\n", res.a0);
		atomic_notifier_call_chain(&optee->notifier, MITEE_CALL_RETURNED, NULL);
		optee_bm_timestamp();
		...
		if (res.a0 == OPTEE_SMC_RETURN_ETHREAD_LIMIT) {
			...
			param.a2 = res.a2;
			param.a3 = res.a3;
			if(DEBUG_CALL) pr_err("receiving rpc call\n");

			memcpy(&save_mask, &(current->cpus_mask), sizeof(struct cpumask));
			if (param.a3 >= NR_CPUS) {
				pr_err("unknown cpu %u\n", param.a3);
			} else {
				set_cpus_allowed_ptr(current, get_cpu_mask(param.a3));
				need_restore_mask = 1;
			}
...
static int auth_ca_name()
{
	...
	if(DEBUG_CALL) pr_err("pid dentry:%s\n",ca_path);

	if(strncmp(ca_path,"/vendor/bin",11) == 0){
		return 0;
	}

	pr_err("auth_ca_name error\n");
	return -1;
}
