...
static unsigned int mitee_memlog_poll(struct file *file, struct poll_table_struct *wait)
{
	struct mitee_memlog_state *s = PDE_DATA(file_inode(file));
	int mask = 0;
...
static ssize_t mitee_memlog_key_read(struct file *file, char __user *buf, size_t size,
			   loff_t *ppos)
{
	struct mitee_memlog_state *s = PDE_DATA(file_inode(file));
	int ret = 0;
	char* psrc = NULL;
	pr_err("enter miteelog mitee_memlog_key_read\n");
...
static unsigned int mitee_memlog_key_poll(struct file *file, struct poll_table_struct *wait)
{
	return 0;
}
...
int mitee_memlog_probe(struct platform_device *pdev)
{
	...
	mrdump_mini_add_extra_file((unsigned long)va, pa, size, "MITEE_LOG");
	s->log = va;
	...
	s->proc_key = proc_create_data("mitee_log_key", 0444, NULL, &mitee_memlog_key_fops,
				     s);
	if (!s->proc_key) {
