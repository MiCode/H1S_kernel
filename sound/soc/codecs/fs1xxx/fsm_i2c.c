...
static int fsm_ext_reset(fsm_dev_t *fsm_dev)
{
	...
	if (!ret && (LOW8(id) == FS1815_DEV_ID || LOW8(id) == FS15XX_DEV_ID)) {
...
#ifdef CONFIG_FSM_CHIPID_CHECK
static int fsm_chipid_check(fsm_dev_t *fsm_dev)
{
	uint16_t id = 0;
	int addr;
	int ret;
	...
	if (ret == 0)
		pr_info("read chipid success: 0x%x", id);
	fsm_dev->i2c->addr = addr;
	fsm_mutex_unlock();
			...
			const struct i2c_device_id *id)
{
	...
	if (fsm_dev == NULL) {
		dev_err(&i2c->dev, "alloc memory failed");
		return -ENOMEM;
	}
...
#ifdef CONFIG_FSM_CHIPID_CHECK
	ret = fsm_chipid_check(fsm_dev);
	if (ret) {
		devm_kfree(&i2c->dev, fsm_dev);
		dev_err(&i2c->dev, "failed to check chip_id");
		return ret;
	}
#endif
...
static void fsm_i2c_shutdown(struct i2c_client *i2c)
{
	...
	if (fsm_dev->is1820 && gpio_is_valid(fsm_dev->rst_gpio))
		gpio_set_value(fsm_dev->rst_gpio, 0);
