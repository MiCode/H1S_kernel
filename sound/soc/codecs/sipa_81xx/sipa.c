	...
	const struct i2c_device_id *id)
{
	...
	if (si_pa->chip_type == CHIP_TYPE_SIA8152X) {
		...
		if (ret < 0) {
			pr_err("[  err][%s] %s: i2c = 0x%02x read fail !!! \r\n",
				LOG_FLAG, __func__, client->addr);

			sipa_regmap_remove(si_pa);

			//Compatible with 0x2a and 0x5a, dtsi is configured as 0x5a
			client->addr -= 0x30;
			si_pa->regmap = sipa_regmap_init(client, si_pa->channel_num,
				get_one_available_chip_type(si_pa->chip_type));
			if (IS_ERR(si_pa->regmap)) {
		...
		pr_info("[ info][%s] %s: i2c = 0x%02x read chip id = 0x%x !!! \r\n",
			LOG_FLAG, __func__, client->addr, value);
	}
	...
	if(!(si_pa->chip_type < CHIP_TYPE_SIA8101 || si_pa->chip_type >= CHIP_TYPE_UNKNOWN)) {
		strcpy(lct_audio_pa_info, dev_name(&client->dev));
		strcat(lct_audio_pa_info, "-");
		strcat(lct_audio_pa_info, support_chip_type_name_table[si_pa->chip_type]);
		strcat(lct_audio_pa_info, "--");
