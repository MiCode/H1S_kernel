...
static int sipa_tuning_read_down(void __user *arg, sipa_sync_t *pdata)
{
    int cnt = 0;
...
static long sipa_tuning_comm_ioctl(unsigned int cmd, unsigned long arg, sipa_sync_t *up, sipa_sync_t *down)
{
    ...
    switch (cmd) {
        ...
        case SIPA_TUNING_CTRL_RD_DOWN: {
                ret = sipa_tuning_read_down((void __user *)arg, down);
...
static int sipa_tuning_tool_open(struct inode *inode, struct file *fp)
{
    ...
    memset(priv->toolup.data, 0, priv->toolup.len);
    memset(priv->tooldown.data, 0, priv->tooldown.len);
